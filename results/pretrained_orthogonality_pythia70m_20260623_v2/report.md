...
- Target modules: `dense_h_to_4h, dense_4h_to_h`; selected count: 6
...
- Hessian/Gauss-Newton proxy cosine heatmap generated for q/s/r over 6 pretrained model modules; the proxy is local activation covariance `X^T X`, not the full model Hessian.
- Higher overlap vs linearized perturbation additivity: Spearman(|rho_H|, |A_ij|) = 0.1249 (n=18). Additivity rows use `W + Delta_i + Delta_j`, while executable order effects are reported separately in `order_gap.csv`.
- Real degradation: Spearman(|rho_H|, PPL degradation) = 0.2941 (n=18); zero-shot degradation = 0.2877 (n=18).
- Taylor/cross-term prediction vs actual loss degradation = -0.0526 (n=18); Frobenius baseline = 0.1290 (n=18); trace-only baseline = -0.1517 (n=18).
- Order gap explanation: R-first conditional overlap = 0.2517 (n=12); singular entropy shift = -0.4685 (n=12); symmetric overlap = 0.0070 (n=12).
- Highest |rho_H| row: L0:dense_4h_to_h pair=sr |rho_H|=0.0843, |A_ij|=0.0235.
- Largest order gap: L3:dense_h_to_4h rq vs qr abs loss gap=0.0359.
- Best compressed strategy by PPL: hessian_layerwise PPL=97.0526, degradation=28.4648; baseline PPL=68.5878.
