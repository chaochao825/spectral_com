...
- Higher overlap vs linearized perturbation additivity: Spearman(|rho_H|, |A_ij|) = 0.4842 (n=36). Additivity rows use `W + Delta_i + Delta_j`, while executable order effects are reported separately in `order_gap.csv`.
- Real degradation: Spearman(|rho_H|, PPL degradation) = 0.2165 (n=36); zero-shot degradation = 0.1164 (n=36).
