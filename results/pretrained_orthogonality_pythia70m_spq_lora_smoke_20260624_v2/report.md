...
- Baseline PPL: 59.8666; NLL: 4.0921; zero-shot mean: 0.5000
...
- Higher overlap vs linearized perturbation additivity: Spearman(|rho_H|, |A_ij|) = 0.2296 (n=36). Additivity rows use `W + Delta_i + Delta_j`, while executable order effects are reported separately in `order_gap.csv`.
- Real degradation: Spearman(|rho_H|, PPL degradation) = 0.1851 (n=36); zero-shot degradation = 0.0316 (n=36).
- Taylor/cross-term prediction vs actual loss degradation = 0.6839 (n=36); Frobenius baseline = 0.4126 (n=36); trace-only baseline = 0.3588 (n=36).
- Order gap explanation: R-first conditional overlap = -0.3096 (n=24); singular entropy shift = 0.3626 (n=24); symmetric overlap = 0.2296 (n=24).
- Highest |rho_H| row: L3:dense pair=qs |rho_H|=0.3515, |A_ij|=1.7527.
- Largest order gap: L5:dense_4h_to_h rq vs qr abs loss gap=0.1181.
- Best compressed strategy by PPL: fixed_qsr_default PPL=217.8215, degradation=157.9550; baseline PPL=59.8666.
...
- No-LoRA comparison: fixed SPQ-like PPL=1194.7278, Hessian-guided-SPQ PPL=259.6927; delta guided-fixed=-935.0351. Both use the same nominal bits/keep/rank budget.
- LoRA-recovered comparison (5 steps, rank 4): fixed SPQ-like PPL=1013.3972, Hessian-guided-SPQ PPL=235.6186; delta guided-fixed=-777.7786.
