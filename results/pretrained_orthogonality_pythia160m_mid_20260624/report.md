...
- Baseline PPL: 45.9731; NLL: 3.8281; zero-shot mean: 0.3750
...
- Higher overlap vs linearized perturbation additivity: Spearman(|rho_H|, |A_ij|) = 0.0654 (n=36). Additivity rows use `W + Delta_i + Delta_j`, while executable order effects are reported separately in `order_gap.csv`.
- Real degradation: Spearman(|rho_H|, PPL degradation) = 0.2932 (n=36); zero-shot degradation = 0.0400 (n=36).
- Taylor/cross-term prediction vs actual loss degradation = 0.4610 (n=36); Frobenius baseline = 0.1012 (n=36); trace-only baseline = 0.1441 (n=36).
- Order gap explanation: R-first conditional overlap = -0.2757 (n=24); singular entropy shift = 0.3635 (n=24); symmetric overlap = -0.0139 (n=24).
- Highest |rho_H| row: L6:dense_4h_to_h pair=qs |rho_H|=0.3947, |A_ij|=0.0311.
- Largest order gap: L11:dense_4h_to_h rq vs qr abs loss gap=0.1184.
- Best compressed strategy by PPL: hessian_layerwise PPL=112.5270, degradation=66.5539; baseline PPL=45.9731.
