...
- Text source: `zero_shot_backup:arc_easy,hellaswag`; split policy: `disjoint_sequential_text_windows` (calib=32, eval=64, recovery=64 texts).
- Compression settings: q=rtn/bits4, s=wanda/keep0.5, r=whitened_svd/rank0.5
- Baseline PPL: 71.5760; NLL: 4.2708; zero-shot mean: nan
...
- Higher overlap vs linearized perturbation additivity: Spearman(|rho_H|, |A_ij|) = 0.2727 (n=12). Additivity rows use `W + Delta_i + Delta_j`, while executable order effects are reported separately in `order_gap.csv`.
- Real degradation: Spearman(|rho_H|, PPL degradation) = 0.1888 (n=12); zero-shot degradation = nan (n=0).
- Taylor/cross-term prediction vs actual loss degradation = 0.3287 (n=12); Frobenius baseline = 0.8811 (n=12); trace-only baseline = 0.7413 (n=12).
- Order gap explanation: R-first conditional overlap = 0.5952 (n=8); singular entropy shift = 0.3810 (n=8); symmetric overlap = 0.1429 (n=8).
- Highest |rho_H| row: L0:query_key_value pair=sr |rho_H|=0.0563, |A_ij|=0.5204.
- Largest order gap: L0:query_key_value rs vs sr abs loss gap=0.0376.
- Best compressed strategy by PPL: hessian_layerwise PPL=133.8312, degradation=62.2552; baseline PPL=71.5760.
...
Text provenance is recorded in `metrics/text_source_metadata.csv`; fair benchmark zero-shot scores are in `metrics/fair_benchmark_zero_shot.csv`. The top-level strategy zero-shot mean is `nan` because `--zero-shot-strategy-limit 0` disabled non-fair strategy zero-shot evaluation for this run.
